...
/**
 * API implentation of emul_mspi_dev_api_transceive transceive.
 *
				       ...
				       uint32_t timeout)
{
	...
	for (uint32_t count = 0; count < num_packet; ++count) {
		...
		if (packet->ui32DeviceAddr > cfg->size ||
		    packet->ui32DeviceAddr + packet->ui32NumBytes > cfg->size) {
			return -ENOMEM;
		}

		if (packet->eDirection == MSPI_RX) {
			memcpy(packet->pui32Buffer, data->mem + packet->ui32DeviceAddr,
			       packet->ui32NumBytes);
		} else if (packet->eDirection == MSPI_TX) {
			memcpy(data->mem + packet->ui32DeviceAddr, packet->pui32Buffer,
			       packet->ui32NumBytes);
		}
...
/**
 * API implentation of flash write.
 *
				 ...
				 const void *wdata, size_t len)
{
	...
	data->xfer.bAsync              = false;
	data->xfer.eMode               = MSPI_DMA;
	data->xfer.ui32TXDummy         = data->dev_cfg.ui32TXDummy;
	data->xfer.ui16InstrLength     = data->dev_cfg.ui16InstrLength;
	data->xfer.ui16AddrLength      = data->dev_cfg.ui16AddrLength;
	data->xfer.bHoldCE             = false;
	data->xfer.ui8Priority         = 1;
	data->xfer.pPacket             = &data->packet;
	data->xfer.ui32NumPacket       = 1;
	...
	while (len) {
		...
		data->packet.eDirection        = MSPI_TX;
		data->packet.ui16DeviceInstr   = (uint16_t)data->dev_cfg.ui32WriteInstr;
		data->packet.ui32DeviceAddr    = offset;
		data->packet.pui32Buffer       = (uint32_t *)src;
		data->packet.ui32NumBytes      = i;
...
/**
 * API implentation of flash read.
 *
 * @param flash Pointer to MSPI flash device.
 * @param offset Flash device address.
		                ...
		                void *rdata, size_t len)
{
	...
	data->packet.eDirection        = MSPI_RX;
	data->packet.ui16DeviceInstr   = (uint16_t)data->dev_cfg.ui32ReadInstr;
	data->packet.ui32DeviceAddr    = offset;
	data->packet.pui32Buffer       = (uint32_t *)rdata;
	data->packet.ui32NumBytes      = len;

	data->xfer.bAsync              = false;
	data->xfer.eMode               = MSPI_DMA;
	data->xfer.ui32RXDummy         = data->dev_cfg.ui32RXDummy;
	data->xfer.ui16InstrLength     = data->dev_cfg.ui16InstrLength;
	data->xfer.ui16AddrLength      = data->dev_cfg.ui16AddrLength;
	data->xfer.bHoldCE             = false;
	data->xfer.ui8Priority         = 1;
...
/**
 * API implentation of flash get_parameters.
 *
 * @param flash Pointer to MSPI flash device.
 *
...
/**
 * API implentation of flash pages_layout.
 *
 * @param flash Pointer to MSPI flash device.
 ...
 */
static int emul_mspi_device_init(const struct emul *emul, const struct device *bus)
{
	const struct flash_mspi_emul_device_config *cfg = emul->dev->config;
	struct flash_mspi_emul_device_data *data = emul->dev->data;
